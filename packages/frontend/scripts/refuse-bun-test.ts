 ...
 * common case. In OxyHQServices that left `bun test` from the repo root
 * uncovered, because its root bunfig serves packages that legitimately use
 * bun's runner. This repo has exactly one package, so the ROOT bunfig.toml
 * carries a second `[test].preload` pointing at this same file, and both entry
 * points are closed.
 *
 * Vendored from OxyHQServices `scripts/refuse-bun-test.ts` when the app moved
 * to its own repo. The hazard is a property of bun's runner, not of any repo
 * layout, so it travelled with the suite.
 */
